...
export default function Chat() {
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    if (data.video) {
      setSpeaking(true);
      const video = document.getElementById("sara-video") as HTMLVideoElement;
      if (video) {
        video.src = data.video;
        video.play();
        video.onended = () => setSpeaking(false);
      }
    } else if (data.audio) {
      ...
      {!started && (
        <div style={{
          ...
          <div style={{ color: "#9ca3af", fontSize: 14, marginBottom: 10 }}>
            Aapki personal AI assistant
          </div>
          <button onClick={async () => {
            // Audio unlock karo Chrome ke liye
            try {
            ...
            border: "none", borderRadius: 30, padding: "18px 50px",
            color: "#fff", fontWeight: 700, fontSize: 18, cursor: "pointer",
            boxShadow: "0 0 40px rgba(255,100,200,0.6)",
            transform: "scale(1)",
      ...
      <div style={{
        ...
        <div style={{
          ...
          <video
            ...
            playsInline
          >
            <source src="/sara.gif" type="video/mp4" />
          </video>
          <img
            src="/sara.gif"
            alt="SARA"
